...
# Esempi simili simulati per demonstration
similar_examples_db = {
    "Amanita phalloides": [
...
# Fattori di calibrazione simulati
calibration_factors = {
    "Amanita phalloides": 0.92,  # Il modello è troppo sicuro
    "Boletus edulis": 1.08,       # Il modello è troppo incerto
    "Amanita muscaria": 0.95,
    "Cantharellus cibarius": 1.05,
    "default": 1.0
}

# Inizializza session state per tracking
...
def assign_experimental_condition(user_id=None):
    """Assegna condizione sperimentale basata su hash dell'utente"""
    if user_id is None:
...
def monte_carlo_uncertainty(model, image_array, n_samples=30):
    """Calcola incertezza con Monte Carlo Dropout simulato"""
    predictions = []
...
def display_gradcam(image, heatmap, alpha=0.4):
    """Visualizza Grad-CAM sovrapposto all'immagine"""
...
def generate_contrastive_explanation(predicted_species, confidenze, species_list):
    """Genera spiegazione contrastiva"""
...
def show_similar_examples(predicted_species):
    """Mostra esempi simili dal training set"""
...
def create_uncertainty_visualization(mean_pred, std_pred, species_list):
    """Crea visualizzazione dell'incertezza"""
...
def advanced_metrics_tracking(image, predictions, user_responses):
    """Traccia metriche avanzate per ricerca"""
...
def log_user_interaction(prediction_data, user_responses, decision_time, explanation_mode, advanced_metrics):
    """Registra l'interazione completa dell'utente"""
    interaction = {
...
def detailed_questionnaire():
    """Questionario dettagliato post-task"""
...
# Assegnazione condizione sperimentale (una sola volta per sessione)
if 'assigned_condition' not in st.session_state:
    st.session_state.assigned_condition = assign_experimental_condition()

# Override manuale per testing (rimuovi in produzione)
if st.sidebar.checkbox("Override condizione (solo per testing)"):
...
st.sidebar.info(f"**Condizione assegnata:** {EXPLANATION_MODES[explanation_mode]}")

# Raccolta dati demografici (una sola volta)
...
if st.session_state.demographic_collected:
    ...
    if uploaded_file is not None:
        ...
        with st.spinner('🔄 Analisi in corso...'):
            ...
            # Calcoli aggiuntivi per spiegazioni avanzate
            ...
            # Grad-CAM (se necessario)
            if explanation_mode in ["gradcam_only", "full_explanation"]:
        ...
        # Spiegazioni basate sulla modalità assegnata
        ...
        if explanation_mode == "no_explanation":
            ...
            confidence_score = confidenze[np.argmax(confidenze)]
            st.write(f"**Confidenza:** {confidence_score:.1f}%")
            superimposed_img = display_gradcam(image, heatmap)
            st.image(superimposed_img, caption='🔍 Aree di focus del modello (Grad-CAM)', use_column_width=True)
            ...
            # Grad-CAM
            superimposed_img = display_gradcam(image, heatmap)
            st.image(superimposed_img, caption='🔍 Grad-CAM - Aree di focus', use_column_width=True)
